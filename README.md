...
| BLE | Nearby device discovery, GATT enumeration, credential/secret scan on readable characteristics |
...
python3 iot_scanner.py --ble-scan
python3 iot_scanner.py --ble-address AA:BB:CC:DD:EE:FF --ble-enum
...
- **BLE vendor-characteristic detection** uses the fact that all Bluetooth
  SIG-assigned 16-bit UUIDs sit inside one fixed 128-bit template
  (`0000XXXX-0000-1000-8000-00805F9B34FB`). Anything outside that template is
  a custom vendor UUID — a better signal for "worth reviewing" than trying to
  guess vendor-specific service UUIDs. The same credential/secret regex used
  on firmware strings is re-applied to readable characteristic values.
